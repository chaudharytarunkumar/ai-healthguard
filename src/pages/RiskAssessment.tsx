...
export default function RiskAssessment() {
 ...
 className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-lg bg-gradient-primary shadow-xl shadow-primary/20"
 >
 <Heart className="h-10 w-10 text-white fill-white/20" />
 </motion.div>
 <h1 className="mb-3 text-4xl font-semibold sm:text-5xl">Risk Assessment</h1>
 ...
 <div className="relative glass-card rounded-[2.5rem] p-8 sm:p-12 shadow-sm">
 {/* Progress bar */}
 <div className="mb-12">
 <div className="flex items-center justify-between mb-4">
 <span className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">Section {currentSectionIdx + 1} of {sections.length}</span>
 ...
 <Label htmlFor={fieldName} className="text-[13px] font-semibold uppercase text-muted-foreground group-focus-within/field:text-primary transition-colors">
 {field.label}
 </Label>
 <TooltipProvider>
 <Tooltip>
 ...
 <TooltipContent side="top" className="max-w-[240px] rounded-xl p-3 border shadow-sm">
 ...
 className={`h-14 rounded-md border-2 bg-muted/30 px-6 font-bold transition-all focus:bg-background focus:ring-4 focus:ring-primary/10 ${errors[fieldName] ?"border-destructive/50" :"border-transparent group-hover/field:border-primary/20 focus:border-primary"}`}
 />
 <span className="absolute right-5 top-1/2 -translate-y-1/2 text-[10px] font-semibold uppercase text-muted-foreground/60">{field.unit}</span>
 </>
 ) : (
 <Select
 value={formData[fieldName] ||""}
 ...
 >
 <SelectTrigger className={`h-14 rounded-md border-2 bg-muted/30 px-6 font-bold transition-all focus:bg-background focus:ring-4 focus:ring-primary/10 ${errors[fieldName] ?"border-destructive/50" :"border-transparent group-hover/field:border-primary/20 focus:border-primary"}`}>
 <SelectValue placeholder="Selection..." />
 </SelectTrigger>
 <SelectContent className="rounded-md border p-1 shadow-sm">
 ...
 <div className="flex gap-4">
 {currentSectionIdx > 0 && (
 <Button type="button" variant="outline" onClick={prevSection} className="h-10 px-4 rounded-md border-2 font-semibold gap-2 transition-all hover:bg-muted">
 <ChevronLeft className="h-5 w-5" /> Back
 </Button>
 )}
 
 {currentSectionIdx < sections.length - 1 ? (
 <Button type="button" onClick={nextSection} className="h-10 px-4 rounded-md bg-primary text-white font-semibold hover:bg-slate-800 gap-2 transition-all hover:translate-x-1 shadow-xl">
 Proceed <ChevronRight className="h-5 w-5" />
 </Button>
 ) : (
 <Button type="submit" className="h-10 px-6 rounded-md bg-gradient-primary text-white font-semibold gap-3 transition-all shadow-md shadow-primary/30">
