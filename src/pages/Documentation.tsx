    ...
    <div className="min-h-screen px-6 py-16 bg-background relative overflow-hidden">
      ...
      >
        <motion.div variants={itemVariants} className="mb-16 text-center">
          ...
          <h1 className="mb-4 text-5xl font-black tracking-tighter text-slate-800 sm:text-6xl">Project Documentation</h1>
        ...
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-3 mb-20">
          {documents.map((doc, i) => (
            ...
            >
              <div className={`mb-10 flex h-16 w-16 items-center justify-center rounded-[1.25rem] transition-all group-hover:scale-110 ${doc.primary ? 'bg-primary text-white shadow-2xl' : 'bg-muted/30 text-muted-foreground group-hover:bg-primary group-hover:text-white'}`}>
              ...
              <h3 className="mb-4 text-2xl font-black tracking-tight text-slate-800 leading-tight">{doc.title}</h3>
              <p className="mb-10 text-sm font-medium leading-relaxed text-muted-foreground flex-1">{doc.desc}</p>
              
              <div className="flex gap-4">
                <Button asChild className={`flex-1 h-14 rounded-2xl font-black gap-3 shadow-xl transition-all active:scale-95 ${doc.primary ? 'bg-gradient-primary text-white shadow-primary/30' : 'bg-slate-100 text-slate-800 hover:bg-slate-200 shadow-none'}`}>
             ...
             {[
                ...
                <div key={idx} className="flex items-center gap-5 p-6 rounded-[2rem] bg-muted/20 border-2 border-transparent">
                    ...
                    <div>
                        ...
                        <p className="font-black text-slate-800">{feat.text}</p>
                    </div>
                </div>
             ))}
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="relative group">
            ...
            <div className="relative glass-card rounded-[3rem] p-5 shadow-elevated overflow-hidden border-2 border-white/50 backdrop-blur-xl">
              <div className="flex flex-col sm:flex-row items-center justify-between p-8 border-b border-muted/30">
                  <div className="flex items-center gap-5 mb-4 sm:mb-0">
                      ...
                      <div>
                        ...
                        <h4 className="font-black text-xl tracking-tighter text-slate-800 mt-1">interactive_report_preview.pdf</h4>
                      </div>
                  </div>
                  <Button variant="outline" className="h-11 px-6 rounded-xl border-2 font-black transition-all hover:bg-primary hover:text-white hover:border-primary active:scale-95 shadow-md bg-white" asChild>
              ...
              <div className="p-10 bg-primary text-center text-[11px] font-bold text-white/40 tracking-widest uppercase">
